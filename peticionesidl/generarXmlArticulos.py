...
def generarXmlArticulos():
    ...
    try:
        ...
        int23 = ET.SubElement(articles, "int23")
        if len(rows) > 0:
            for r in rows:
                crearInt23Rub110(r, int23, articles, cx)
        ...
        #result = "<articles_response><int03><rub110><activity_code>GNS</activity_code><item_code>8435002184663</item_code><status>OK</status></rub110><rub110><activity_code>GNS</activity_code><item_code>8435002184670</item_code><status>OK</status></rub110><rub110><activity_code>GNS</activity_code><item_code>8435002184694</item_code><status>OK</status></rub110></int03></articles_response>"
        if not result:
            res[0] = False
            res[1] = result
            print(result)
            ...
            for child in root.findall('int03/rub110'):
                ...
                if referencia and referencia != "":
                    if status == "OK":
                        ...
                        cx["cur"].execute("SELECT id FROM idl_erroneos WHERE tipo IN ('IDL_PEDIDOS_CLI','IDL_PEDIDOS_PROV','IDL_VIAJES_DEST','IDL_VIAJES_ORI','IDL_VIAJES_TRANS_OR','IDL_PEDIDOS_CRO','IDL_VIAJES_CRO') AND motivo IN (SELECT referencia FROM atributosarticulos WHERE barcode = '" + referencia + "')")

                        errores = cx["cur"].fetchall()
                        if len(errores) > 0:
                            for er in errores:
                                 if er["id"]:
                                    cx["cur"].execute("DELETE FROM idl_erroneos WHERE tipo IN ('IDL_PEDIDOS_CLI','IDL_PEDIDOS_PROV','IDL_VIAJES_DEST','IDL_VIAJES_ORI','IDL_VIAJES_TRANS_OR','IDL_PEDIDOS_CRO','IDL_VIAJES_CRO') AND motivo IN (SELECT referencia FROM atributosarticulos WHERE barcode = '" + referencia + "')")
                                    cx["conn"].commit()
                    else:
                        ...
                        cx["conn"].commit()
                        registraError(tipo, referencia, error, cx)
...
def crearCabeceraXMLArticulo(r, int03, cx):
    ...
    ET.SubElement(rub110, "expiry_group_code").text = expiryGroup
    ...
    if not peso or peso == "None":
        peso = "1.0";

    if not alto or alto == "None":
        alto = "0.0";

    if not largo or largo == "None":
        largo = "0.0";

    if not ancho or ancho == "None":
        ancho = "0.0";

    if not volumen or volumen == "None":
        volumen = "0.0";

    ET.SubElement(rub120, "net_weight").text = str(round(float(peso), 2))
    ET.SubElement(rub120, "gross_weight").text = str(round(float(peso), 2))
    ET.SubElement(rub120, "height").text = str(round(float(alto), 2))
    ET.SubElement(rub120, "width").text = str(round(float(ancho), 2))
    ET.SubElement(rub120, "depth").text = str(round(float(largo), 2))
    ET.SubElement(rub120, "volume").text = str(round(float(volumen) / 1000, 2))
...
def crearInt23Rub110(r, int23, articles, cx):
    rub110 = ET.SubElement(int23, "rub110")
