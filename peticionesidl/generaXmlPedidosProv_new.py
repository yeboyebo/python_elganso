...
def generaXmlPedidos():
    ...
    cx["cur"].execute("SELECT nombre FROM eg_fichprocesados WHERE tipo = 'IDL_PEDIDOS_PROV'")
    rows = cx["cur"].fetchall()
    if len(rows) > 0:
        return True

    cx["cur"].execute("INSERT INTO eg_fichprocesados (estado,hora,tipo,nombre,fecha) VALUES ('En proceso',CURRENT_TIME,'IDL_PEDIDOS_PROV','IDL_PEDIDOS_PROV',CURRENT_DATE)")
    cx["conn"].commit()
    ...
    try:
        ...
        cx["cur"].execute("SELECT pp.codigo AS codigo, pp.idpedido AS idpedido, pp.fechaentrada AS fechaentrada, pp.fecha AS fecha, pp.enviomodificado AS enviomodificado, ai.codalmacenidl AS codalmacen, ai.nombre AS nombrealmacen, pp.codproveedor AS codproveedor, MAX(pr.numenvio) AS numenvio FROM pedidosprov pp INNER JOIN almacenesidl ai ON pp.codalmacen = ai.codalmacen INNER JOIN idl_proveedores ip ON (pp.codproveedor = ip.codproveedor AND ip.ok = true) LEFT OUTER JOIN eg_pedidosrecibidos pr ON pp.idpedido = pr.idpedido WHERE (pp.enviado = true AND pp.fichero is NULL AND pp.codproveedor IS NOT NULL AND pp.idpedido IN (SELECT lpp.idpedido FROM lineaspedidosprov lpp WHERE lpp.idpedido = pp.idpedido)) GROUP BY pr.idpedido, pp.codigo, pp.idpedido, pp.fechaentrada, pp.fecha, pp.enviomodificado, ai.codalmacenidl, ai.nombre, pp.codproveedor ORDER BY pp.fecha ASC LIMIT 1")
        ...
        if len(rows) > 0:
            for p in rows:
                ...
                cx["cur"].execute("SELECT lpp2.referencia as reflinea, ia.referencia as refidl FROM lineaspedidosprov lpp2 LEFT JOIN idl_articulos ia ON lpp2.referencia = ia.referencia WHERE lpp2.idpedido = " + str(idPedido) + " AND ((ia.referencia IS NULL) OR (ia.referencia IS NOT NULL AND ia.ok = false))")
                ...
                if len(rowsArt) > 0:
                    ...
                    for art in rowsArt:
                        ...
                        cx["conn"].commit()
                        cx["cur"].execute("DELETE FROM eg_fichprocesados WHERE tipo = 'IDL_PEDIDOS_PROV'")
                        cx["conn"].commit()
                        return True
                if faltaArticulos:
                    cx["cur"].execute("DELETE FROM eg_fichprocesados WHERE tipo = 'IDL_PEDIDOS_PROV'")
...
def creaXmlRecepcionPedido(p, int15, cx):
    cx["cur"].execute("SELECT l.barcode as barcode, l.cantidad-l.totalenalbaran as cantidad, l.idlinea as idlinea, l.descripcion as descripcion, p.precioetiqueta as precioetiqueta FROM lineaspedidosprov l LEFT OUTER JOIN preciosetiquetadopedidosprov P ON l.idlinea = p.idlinea WHERE l.cantidad-l.totalenalbaran > 0 and l.idpedido = " + str(p["idpedido"]))
    rows = cx["cur"].fetchall()
    numL = 1
    if len(rows) <= 0:
        return False
    ...
    for l in rows:
        ...
        precio = formateaPrecio(l["precioetiqueta"])
        ET.SubElement(rub120, "batch_1").text = "EUR" + precio
...
def formateaPrecio(precio):
    if not precio:
        return "000000.00"
    else:
        precio = "{:0>9.2f}".format(precio, 2)
